(function () {
    ...
    function temperatureClass(value) {
        ...
        if (value >= 35) { return 'aifs-temp-extreme'; }
        if (value >= 30) { return 'aifs-temp-hot'; }
        if (value >= 22) { return 'aifs-temp-warm'; }
        if (value >= 12) { return 'aifs-temp-mild'; }
        if (value >= 4) { return 'aifs-temp-cool'; }
        return 'aifs-temp-cold';
    }

    function gustClass(value) {
        if (!finite(value)) { return ''; }
        if (value >= 80) { return 'aifs-gust-danger'; }
        if (value >= 60) { return 'aifs-gust-strong'; }
        if (value >= 40) { return 'aifs-gust-moderate'; }
        return '';
    }

    function windForceClass(value) {
        if (!finite(value)) { return 'aifs-wind-calm'; }
        if (value >= 80) { return 'aifs-wind-violent'; }
        if (value >= 60) { return 'aifs-wind-strong'; }
        if (value >= 40) { return 'aifs-wind-fresh'; }
        if (value >= 20) { return 'aifs-wind-moderate'; }
        return 'aifs-wind-light';
    }

    function rainClass(value) {
        if (!finite(value) || value < 0.1) { return ''; }
        if (value >= 15) { return 'aifs-rain aifs-rain-intense'; }
        if (value >= 5) { return 'aifs-rain aifs-rain-strong'; }
        if (value >= 2) { return 'aifs-rain aifs-rain-moderate'; }
        if (value >= 0.5) { return 'aifs-rain aifs-rain-light'; }
        return 'aifs-rain aifs-rain-trace';
    }
    ...
    function chartDayBands(svg, count, margin, innerWidth, innerHeight, meta) {
        ...
        while (start < count) {
            ...
            svg.appendChild(svgElement('rect', {
                ...
                height: innerHeight,
                class: 'aifs-chart-day-band aifs-dayband-' + token
            }));
            ...
            svg.appendChild(svgElement('rect', {
                ...
                width: Math.max(0, endX - startX),
                height: 46,
                class: 'aifs-chart-day-strip aifs-daystrip-' + token
            }));
            ...
            if (start > 0) {
                svg.appendChild(svgElement('line', {
                    ...
                    class: 'aifs-chart-day-separator aifs-dayseparator-' + token
                }));
            }
            ...
            if (endX - startX > 72) {
                ...
                svg.appendChild(svgElement('text', {
                    ...
                    y: margin.top + 31,
                    'text-anchor': 'middle',
                    class: 'aifs-chart-day-label aifs-daylabel-' + token
                }, dayLabel));
            }
            start = end + 1;
        }
    ...
    function appendChartMetrics(container, metrics) {
        ...
        block.className = 'aifs-chart-metrics';
        valid.forEach(function (metric) {
            var item = document.createElement('span');
            item.className = 'aifs-chart-metric';
            item.appendChild(textSpan(metric.label, 'aifs-chart-metric-label'));
            item.appendChild(textSpan(metric.value, 'aifs-chart-metric-value'));
    ...
    function appendChartViewport(container, svg) {
        ...
        viewport.className = 'aifs-chart-viewport';
        var tooltip = document.createElement('div');
        tooltip.className = 'aifs-chart-tooltip';
        tooltip.hidden = true;
        ...
        function tooltipTarget(node) {
            ...
            return node.closest('[data-aifs-tooltip]');
        }
        ...
        viewport.addEventListener('pointerover', function (event) {
            ...
            if (!target) { return; }
            tooltip.textContent = target.getAttribute('data-aifs-tooltip') || '';
    ...
    function drawExtremaLabels(svg, points, values, formatter, className) {
        ...
        indexes.forEach(function (index) {
            ...
            svg.appendChild(svgElement('text', {
                ...
                y: y,
                'text-anchor': 'middle',
                class: 'aifs-chart-value-label ' + className
    ...
    function renderLineChart(container, labels, series, options) {
        ...
        if (!scale) {
            container.appendChild(textSpan('Données indisponibles pour ce diagramme.', 'aifs-chart-empty'));
            return;
        }
        ...
        svg.classList.add('aifs-chart-svg');
        ...
        for (var g = 0; g <= tickValue; g += 1) {
            ...
            svg.appendChild(svgElement('line', {
                ...
                y1: gy,
                y2: gy,
                class: 'aifs-chart-grid'
            }));
            svg.appendChild(svgElement('text', {
                x: margin.left - 9,
                y: gy + 4,
                'text-anchor': 'end',
                class: 'aifs-chart-axis'
        ...
        chartTickIndexes(n, 8).forEach(function (index) {
            ...
            svg.appendChild(svgElement('line', {
                ...
                y1: margin.top,
                y2: margin.top + innerHeight,
                class: 'aifs-chart-grid aifs-chart-grid-vertical'
            }));
            svg.appendChild(svgElement('text', {
                x: x,
                y: height - 18,
                'text-anchor': index === 0 ? 'start' : (index === n - 1 ? 'end' : 'middle'),
                class: 'aifs-chart-axis aifs-chart-xaxis'
            }, axisLabel));
        });
        ...
        series.forEach(function (item, seriesIndex) {
            ...
            if (series.length === 1 && options.fillArea) {
                segments.forEach(function (segment) {
                    ...
                    svg.appendChild(svgElement('path', {
                        d: d,
                        class: 'aifs-chart-area ' + item.className
                    }));
                });
            }
            ...
            segments.forEach(function (segment) {
                svg.appendChild(svgElement('path', {
                    ...
                    class: 'aifs-chart-line ' + item.className,
                    fill: 'none'
                }));
            });
            ...
            points.forEach(function (point, index) {
                ...
                var attrs = {
                    ...
                    class: (options.showPoints === false ? 'aifs-chart-hitpoint ' : 'aifs-chart-point ') + item.className,
                    tabindex: 0,
                    'aria-label': tooltipLabel,
                    'data-aifs-tooltip': tooltipLabel
                };
        ...
        if (options.bandBetween && seriesPoints.length >= 2) {
            ...
            if (upperPoints.length > 1 && upperPoints.length === lowerPoints.length) {
                ...
                var envelope = svgElement('path', {
                    d: area,
                    class: 'aifs-chart-wind-envelope'
                });
                var firstLine = svg.querySelector('.aifs-chart-line');
        ...
        if (series.length > 1) {
            var legend = svgElement('g', { class: 'aifs-chart-legend' });
            series.forEach(function (item, index) {
                ...
                legend.appendChild(svgElement('text', {
                    x: x + 36,
                    y: 21,
                    class: 'aifs-chart-axis aifs-chart-legend-text'
                }, item.label));
            });
    ...
    function rainVisualClass(value) {
        if (!finite(value) || value < 0.1) { return 'aifs-chart-rain-zero'; }
        if (value >= 15) { return 'aifs-chart-rain-intense'; }
        if (value >= 5) { return 'aifs-chart-rain-strong'; }
        if (value >= 2) { return 'aifs-chart-rain-moderate'; }
        if (value >= 0.5) { return 'aifs-chart-rain-light'; }
        return 'aifs-chart-rain-trace';
    }
    ...
    function renderRainChart(container, labels, rainValues, cumulativeValues, options) {
        ...
        for (var cg = 0; cg <= cumTickCount; cg += 1) {
            ...
            svg.appendChild(svgElement('text', {
                ...
                y: cy + 5,
                'text-anchor': 'start',
                class: 'aifs-chart-axis aifs-chart-axis-cumulative'
        ...
        rainValues.forEach(function (value, index) {
            ...
            var bar = svgElement('rect', {
                ...
                class: 'aifs-chart-rain-bar ' + rainVisualClass(value),
                tabindex: 0,
                'aria-label': rainTooltip,
                'data-aifs-tooltip': rainTooltip
            });
            svg.appendChild(bar);
        });
        ...
        if (cleanPoints.length) {
            svg.appendChild(svgElement('path', {
                ...
                class: 'aifs-chart-line aifs-series-rain-cumulative',
                fill: 'none'
            }));
            cumulativePoints.forEach(function (point, index) {
                ...
                var hitPoint = svgElement('circle', {
                    ...
                    cy: point.y,
                    r: 11,
                    class: 'aifs-chart-hitpoint aifs-series-rain-cumulative',
                    tabindex: 0,
                    'aria-label': cumulativeTooltip,
                    'data-aifs-tooltip': cumulativeTooltip,
                    fill: 'transparent',
                    stroke: 'none',
                    opacity: '0'
                });
        ...
        legend.appendChild(svgElement('rect', {
            ...
            height: 11,
            rx: 4,
            class: 'aifs-chart-rain-bar aifs-chart-rain-light'
        }));
        legend.appendChild(svgElement('text', {
            x: margin.left + 28,
            y: 21,
    ...
    function initApp(app) {
        ...
        var hours = Math.max(1, Math.min(51, parseInt(app.dataset.hours || '51', 10)));
        var timezone = app.dataset.timezone || 'Europe/Paris';
        var titlePrefix = app.dataset.titlePrefix || 'Prévisions AROME-IFS';

        var input = app.querySelector('.aifs-city-input');
        var locateButton = app.querySelector('[data-aifs-locate]');
        var results = app.querySelector('.aifs-search-results');
        var status = app.querySelector('.aifs-search-status');
        var generalBody = app.querySelector('[data-aifs-body-general]');
        var stormBody = app.querySelector('[data-aifs-body-storms]');
        var stormSummary = app.querySelector('[data-aifs-storm-summary]');
        var snowBody = app.querySelector('[data-aifs-body-snow]');
        var snowSummary = app.querySelector('[data-aifs-snow-summary]');
        var tabs = Array.prototype.slice.call(app.querySelectorAll('[data-aifs-tab]'));
        var panels = Array.prototype.slice.call(app.querySelectorAll('[data-aifs-panel]'));
        var title = app.querySelector('[data-aifs-title]');
        var altitudeLine = app.querySelector('[data-aifs-altitude]');
        var meta = app.querySelector('[data-aifs-meta]');
        var generated = app.querySelector('[data-aifs-generated]');
        var stale = app.querySelector('[data-aifs-stale]');
        var stormTopScroll = app.querySelector('[data-aifs-top-scroll="storms"]');
        var stormScrollWrap = app.querySelector('[data-aifs-scroll-wrap="storms"]');
        var stormTable = app.querySelector('.aifs-storm-table');
        var snowTopScroll = app.querySelector('[data-aifs-top-scroll="snow"]');
        var snowScrollWrap = app.querySelector('[data-aifs-scroll-wrap="snow"]');
        var snowTable = app.querySelector('.aifs-snow-table');
        var chartTemperature = app.querySelector('[data-aifs-chart-temperature]');
        var chartPressure = app.querySelector('[data-aifs-chart-pressure]');
        var chartRain = app.querySelector('[data-aifs-chart-rain]');
        var chartWind = app.querySelector('[data-aifs-chart-wind]');
        var chartTitleTemperature = app.querySelector('[data-aifs-chart-title-temperature]');
        var chartTitlePressure = app.querySelector('[data-aifs-chart-title-pressure]');
        var chartTitleRain = app.querySelector('[data-aifs-chart-title-rain]');
        var chartTitleWind = app.querySelector('[data-aifs-chart-title-wind]');
        var rainTotal = app.querySelector('[data-aifs-rain-total]');
        ...
        function makeDayCell(date, rowSpan) {
            var cell = createCell('th', 'Date', 'aifs-day-cell aifs-day-' + weekdayToken(date));
            cell.scope = 'rowgroup';
            cell.rowSpan = rowSpan;
            cell.appendChild(textSpan(weekdayFormat.format(date), 'aifs-day-weekday'));
            cell.appendChild(textSpan(shortDateFormat.format(date), 'aifs-day-date'));
            return cell;
        }
        ...
        function setStatus(message, error) {
            ...
            status.classList.toggle('aifs-search-error', Boolean(error));
        }

        function setActiveView(view) {
            tabs.forEach(function (button) {
                var active = button.dataset.aifsTab === view;
                button.classList.toggle('is-active', active);
                button.setAttribute('aria-selected', active ? 'true' : 'false');
            });
            panels.forEach(function (panel) {
                panel.hidden = panel.dataset.aifsPanel !== view;
            });
            ...
            if (view === 'map' && selectedMapFocus) {
                window.requestAnimationFrame(function () {
                    var mapApp = app.querySelector('[data-aifsm-app]');
                    if (mapApp) {
                        mapApp.dispatchEvent(new CustomEvent('aifsm:focus-location', {
        ...
        tabs.forEach(function (button) {
            button.addEventListener('click', function () {
                setActiveView(button.dataset.aifsTab || 'general');
            });
        });
        ...
        function putMessage(body, message, error, colspan) {
            ...
            var cell = createCell('td', '', error ? 'aifs-loading aifs-load-error' : 'aifs-loading');
        ...
        function showTableMessage(message, error) {
            ...
            if (stormSummary) {
                ...
                stormSummary.classList.toggle('aifs-storm-summary-warning', Boolean(error));
            }
            if (snowSummary) {
                snowSummary.textContent = message;
                snowSummary.classList.toggle('aifs-snow-summary-warning', Boolean(error));
            }
        }
            ...
            return fetchJson(baseUrl + '/index.json', { cache: 'no-cache' })
                .then(function (payload) {
                    if (!payload || payload.status !== 'ok' || !payload.departments) {
                        throw new Error('Index national AROME-IFS invalide');
                    }
                    indexData = payload;
                    ...
                    meta.textContent = 'Run du ' + run + ' • résolution ' + resolution +
                        ((payload.model && payload.model.storm_diagnostics) ? ' • diagnostics orages + neige AROME-IFS' : '');
        ...
        function loadDepartment(code) {
            ...
            if (!indexData || !indexData.departments[normalized]) {
                return Promise.reject(new Error('Département hors couverture AROME-IFS'));
            }
        ...
        function renderForecast(departmentData, commune) {
            ...
            function appendHazard(row, label, code) {
                ...
                var cell = createCell('td', label, numeric === null ? '' : 'aifs-hazard aifs-hazard-' + numeric);
            ...
            if (!forecasts.length) {
                showTableMessage('Aucune échéance AROME-IFS disponible pour cette commune.', true);
                return;
            }
            ...
            forecasts.forEach(function (step, currentIndex) {
                ...
                if (dayKey !== previousGeneralDay) {
                    row.classList.add('aifs-new-day');
                    row.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousGeneralDay = dayKey;
                }

                var hourCell = createCell('td', 'Heure', 'aifs-hour aifs-hour-' + weekdayToken(date));
                ...
                var conditionCell = createCell('td', 'Temps', 'aifs-condition');
                conditionCell.appendChild(textSpan(condition.icon, 'aifs-icon'));
                conditionCell.appendChild(textSpan(condition.label));
                row.appendChild(conditionCell);

                var temperatureCell = createCell('td', 'Température', 'aifs-temperature ' + temperatureClass(temp));
                ...
                var windCell = createCell('td', 'Vent', 'aifs-wind-cell ' + windForceClass(windDisplay));
                ...
                if (dayKey !== previousStormDay) {
                    stormRow.classList.add('aifs-new-day');
                    stormRow.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousStormDay = dayKey;
                }
                var stormHour = createCell('td', 'Heure', 'aifs-hour aifs-hour-' + weekdayToken(date));
                ...
                var thunderCell = createCell('td', 'Risque orage', thunder ? 'aifs-thunder-risk aifs-thunder-' + Number(thunderCode) : '');
                if (thunder) {
                    thunderCell.appendChild(textSpan(thunder.icon + ' ', 'aifs-risk-icon'));
                    thunderCell.appendChild(textSpan(thunder.label, 'aifs-risk-label'));
                } else {
                ...
                var capeCell = createCell('td', 'CAPE', finite(cape) && cape >= 1500 ? 'aifs-cape-high' : (finite(cape) && cape >= 500 ? 'aifs-cape-mid' : ''));
                capeCell.textContent = finite(cape) && Number(cape) >= 25 ? formatNumber(cape, 0) + ' J/kg' : '—';
                capeCell.title = 'CAPE instantanée directement produite par AROME-IFS ; les valeurs quasi nulles ne sont pas affichées.';
                stormRow.appendChild(capeCell);
                appendNumber(stormRow, 'LCL', value(values, 'lcl_m'), 0, ' m');

                var lightning = value(values, 'lightning_score');
                appendNumber(stormRow, 'Foudre', lightning, 0, '/100', finite(lightning) && lightning >= 60 ? 'aifs-lightning-high' : '');
                ...
                var stormTypeCell = createCell('td', 'Type d’orage', 'aifs-storm-type');
                ...
                var detailsCell = createCell('td', 'Détails', 'aifs-details-cell');
                var detailsButton = document.createElement('button');
                detailsButton.type = 'button';
                detailsButton.className = 'aifs-details-button';
                detailsButton.textContent = 'Afficher';
                detailsButton.setAttribute('aria-expanded', 'false');
                detailsCell.appendChild(detailsButton);

                var advanced = document.createElement('div');
                advanced.className = 'aifs-advanced-list aifs-details-inline';
                ...
                [
                    ...
                    box.className = 'aifs-advanced-item';
                    box.appendChild(textSpan(item[0], 'aifs-advanced-label'));
                    box.appendChild(textSpan(finite(item[1]) ? formatNumber(item[1], item[2]) + item[3] : '—', 'aifs-advanced-value'));
                ...
                if (dayKey !== previousSnowDay) {
                    snowRow.classList.add('aifs-new-day');
                    snowRow.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousSnowDay = dayKey;
                }
                var snowHour = createCell('td', 'Heure', 'aifs-hour aifs-hour-' + weekdayToken(date));
                ...
                var snowRiskCell = createCell('td', 'Risque neige', snowRisk ? 'aifs-snow-risk aifs-snow-' + Number(snowRiskCode) : '');
                if (snowRisk) {
                    snowRiskCell.appendChild(textSpan(snowRisk.icon + ' ', 'aifs-risk-icon'));
                    snowRiskCell.appendChild(textSpan(snowRisk.label, 'aifs-risk-label'));
                ...
                var stickCell = createCell('td', 'Tenue', finite(stickCode) ? 'aifs-stick aifs-stick-' + Number(stickCode) : '');
                ...
                var snowWindCell = createCell('td', 'Vent moyen / rafales', 'aifs-snow-wind ' + windForceClass(gustDisplay));
                ...
                var snowDetailsCell = createCell('td', 'Détails', 'aifs-details-cell aifs-snow-details-cell');
                var snowDetailsButton = document.createElement('button');
                snowDetailsButton.type = 'button';
                snowDetailsButton.className = 'aifs-details-button';
                ...
                snowAdvanced.className = 'aifs-snow-profile aifs-details-inline';
                snowAdvanced.hidden = true;

                function appendProfileSection(titleText, items) {
                    var section = document.createElement('section');
                    section.className = 'aifs-profile-section';
                    section.appendChild(textSpan(titleText, 'aifs-profile-title'));
                    var list = document.createElement('div');
                    list.className = 'aifs-profile-list';
                    items.forEach(function (item) {
                        var profileItem = document.createElement('div');
                        profileItem.className = 'aifs-profile-item';
                        profileItem.appendChild(textSpan(item[0], 'aifs-profile-label'));
                        profileItem.appendChild(textSpan(finite(item[1]) ? formatNumber(item[1], item[2]) + item[3] : '—', 'aifs-profile-value'));
                ...
                appendProfileSection('Sorties et diagnostics AROME-IFS', [
            ...
            if (stormSummary) {
                stormSummary.classList.toggle('aifs-storm-summary-warning', !hasStormColumns);
                if (!hasStormColumns) {
                    ...
                    stormSummary.textContent = 'Risque orageux maximal sur les ' + forecasts.length + ' prochaines échéances : ' + maxRisk.icon + ' ' + maxRisk.label + ' • CAPE AROME-IFS : risque indicatif, réflectivité indisponible.';
                }
            }
            if (snowSummary) {
                snowSummary.classList.toggle('aifs-snow-summary-warning', !hasSnowColumns);
                if (!hasSnowColumns) {
                    ...
                    snowSummary.textContent = 'Risque de neige maximal sur les ' + forecasts.length + ' prochaines échéances : ' + maxSnow.icon + ' ' + maxSnow.label + ' • cumul neige AROME-IFS 0,025°.';
                }
            }
            ...
            if (altitudeLine) {
                altitudeLine.textContent = finite(altitude)
                    ? 'Altitude de ' + cityName + ' : ≈ ' + formatNumber(altitude, 0) + ' m (point de grille AROME-IFS)'
                    : 'Altitude de ' + cityName + ' : — (relancez le workflow GitHub AROME-IFS v1.0.0)';
                altitudeLine.classList.toggle('aifs-altitude-missing', !finite(altitude));
            }
            selectedMapFocus = {
            ...
            renderLineChart(chartTemperature, chartLabels, [{ label: 'Température', values: chartTemps, className: 'aifs-series-temperature' }], {
                decimals: 0,
                unit: '°C',
                fillArea: false,
            ...
            renderLineChart(chartPressure, chartLabels, [{ label: 'Pression', values: chartPressures, className: 'aifs-series-pressure' }], {
                decimals: 0,
                unit: 'hPa',
                fillArea: false,
                metricMode: 'pressure',
            ...
            renderLineChart(chartWind, chartLabels, [
                { label: 'Vent moyen', values: chartWindValues, className: 'aifs-series-wind' },
                { label: 'Rafales', values: chartGustValues, className: 'aifs-series-gust' }
            ], {
                decimals: 0,
                unit: 'km/h',
                forceZero: true,
        ...
        function selectCommune(candidate) {
            ...
            loadDepartment(candidate.codeDepartement)
                .then(function (departmentData) {
                    ...
                    if (!commune) {
                        throw new Error('Commune absente du catalogue AROME-IFS');
                    }
        ...
        function displaySearchResults(candidates) {
            ...
            candidates.forEach(function (candidate, position) {
                ...
                button.type = 'button';
                button.className = 'aifs-search-result';
                button.id = input.id + '-option-' + position;
                button.setAttribute('role', 'option');
                button.setAttribute('aria-selected', 'false');

                var name = textSpan(candidate.nom, 'aifs-result-name');
        ...
        if (!baseUrl) {
            showTableMessage('Adresse des données AROME-IFS non configurée.', true);
            return;
        }
        loadIndex()
            .then(function () {
    ...
    whenReady(function () {
        document.querySelectorAll('[data-aifs-app]').forEach(initApp);
    });
}());
